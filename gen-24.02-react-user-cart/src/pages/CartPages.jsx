...
const CartPage = () => {
    ...
    useEffect(() => {
        const fetchCart = async () => {
            try {
                ...
                dispatch(loadCart(data.cart_item));
            } catch (error) {
    ...
    const calculateTotalPrice = (cart) => {
        return cart.reduce((total, product) => total + product.originalPrice * product.quantity, 0);
    };
                            ...
                            <tbody>
                                {cart.map((product, index) => (
                                    <tr key={index}>
                                        ...
                                        <td className="border-b-2 border-gray-300 px-4 py-2">${calculateSubtotal(product).toFixed(1)}</td>
                                    </tr>
                                ))}
                                <tr>
                                    <td colSpan="5" className="text-left font-semibold px-4 py-2">Total</td>
                                    <td className="px-4 py-2 font-bold">${totalPrice.toFixed(1)}</td>
                                </tr>
                            </tbody>
                        </table>
