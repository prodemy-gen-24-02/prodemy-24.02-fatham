...
const ProductCard = ({ product }) => {
    ...
    return (
        <div className='border p-2 rounded shadow-md w-full max-w-sm mx-auto hover:scale-105'>
            ...
            <div className='flex'>
                <button className='ml-2 mt-2 bg-clip-border p-3 bg-sky-400 hover:outline' onMouseMove={() => changePhoto(product.image2)}></button>
                <button className='ml-2 mt-2 bg-clip-border p-3 bg-red-200 hover:outline' onMouseMove={() => changePhoto(product.image)}></button>
            </div>
        </div>
    );
};
